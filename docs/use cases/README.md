...
@startuml
    ...
    <font color="red"> AccountAlreadyExistException</font>;
...
@startuml
    ...
    |Система|
    : Перевіряє наявність облікового запису користувача
    <font color="red"> InvalidCredentialsException</font>
    <font color="red"> UserAlreadyExistException</font>;
...
@startuml
    ...
    |Система|
    : Перевіряє наявність облікового запису користувача
    <font color="red"> InvalidUserException</font>;
...
@startuml
    ...
    |Система|
    : Перевіряє наявність облікового запису користувача
    <font color="red"> NullInstanceException</font>;
...
| Виключні ситуації | <font color="Maroon">NullReferenceException</font> - користувач не заповнив необхідні поля. </br> AccessDeniedException - користувач не має відповідних прав. </br> <font color="Maroon">NameAlreadyExistsException</font> - проект з такою назвою вже існує. |
...
if (Користувач має відповідні права?) then (Так)
    |Система|
    :Користувач заповнює поля з даними проекту;
    if (Користувач заповнив усі поля?) then (Так)
        :Система створює проект;
        :Показує його на головній сторінці тімліда та прив'язаних до нього користувачів;
        stop;
    else (Ні)
        |Система|
        :Користувач не заповнив необхідні поля;
        stop;
    endif
else (Ні)
    |Система|
...
if (Користувач авторизований і має відповідні права?) then (Так)
    |Система|
    :Користувач змінює поля з даними проекту;
    if (Користувач заповнив усі поля?) then (Так)
        :Система змінює вказані дані;
        |Користувач|
        :Користувач натискає кнопку "Змінити";
        |Система|
        :Оновлення сторінки та відображення актуальних даних;
        stop;
    else (Ні)
        |Система|
        :Користувач не заповнив необхідні поля;
...
|Користувач|
start;
:Користувач обирає проект та натискає кнопку "Видалити проект";
if (Користувач авторизований і має необхідні права?) then (Так)
    |Система|
    :Система відображає діалогове вікно з запитом ввести назву проекту для підтвердження видалення;
    if (Користувач ввів правильну назву проекту?) then (Так)
        |Користувач|
        :Користувач натискає кнопку "Підтвердити";
        |Система|
        :Система видаляє проект та сповіщує колабораторів проекту;
        :Проект видалено;
        stop;
    else (Ні)
        |Система|
        :Користувач ввів неправильне ім'я проекту;
        stop;
    endif
else (Ні)
    |Система|
...
| Виключні ситуації |  <font color="Maroon">AccessDeniedException</font> - користувач не має відповідних прав </br> <font color="Maroon">NullReferenceException</font> - користувач не заповних необхідні поля </br> <font color="Maroon">NoUserFoundException</font> - користувач ввів імʼя нового тімліда неправильно </br> <font color="Maroon">CancelTokenException</font> - користувач відмінив операцію |
...
| Виключні ситуації |  <font color="Maroon">NullReferenceException</font> - не заповнені обов'язкові поля. </br> <font color="Maroon">CancelTokenException</font> - операцію скасовано користувачем </br> <font color="Maroon">NoResponseException</font> - система не отримала відповідь від адміністратора протягом певного часу|
